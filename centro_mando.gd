...
func _ready() -> void:
	...
	var status: Label = shell["status"] as Label
	...
	var btn_profile: Button = make_button("Perfil", COLOR_PANEL_ALT)
	...
	_add_module(grid, "MISIONES", "Operaciones y recompensas", "res://misiones.tscn")
...
func _add_module(
	...
	var button: Button = make_button("Abrir", COLOR_PANEL_ALT)
...
func _load_user_info() -> void:
	var nombre: String = str(
		APIManager.usuario_actual.get(
			"nombre",
			APIManager.usuario_actual.get("username", "Comandante")
		)
	)
	var poder: String = str(APIManager.usuario_actual.get("poder", 0))
	var alliance_id: Variant = APIManager.usuario_actual.get("alliance_id", null)
